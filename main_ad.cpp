...
void printData(const C& c, const Ttype T, const std::size_t order)
{
 ...
 if(!order){
    ...
    auto message = order?
        std::string("d").append(std::to_string(order)).append("\\alpha/").append("dT"):std::string("alpha values");
    ...
    if(p<8)std::cout<< std::setprecision(p);
    else std::cout<<std::scientific;
